...
class Assets
{
	...
	**/
	...
	public static var bufferBullshit:AudioBuffer = null;
	/**
	...
	**/
	public static function getSound(id:String, useCache:Bool = true):Sound
	{
		...
		// FUCK YOU ASSHOLE (don)
		if (exists(id, SOUND))
			bufferBullshit = LimeAssets.getAudioBuffer(id, false);

		if (bufferBullshit != null)
		{
			#if flash
			var sound = bufferBullshit.src;
			#else
			var sound = Sound.fromAudioBuffer(bufferBullshit);
			bufferBullshit = null; // PLEASE!!!! STOP PLAYING THAT CONFIRM SOUND, GET OUT OF MY HEAD!!!!!!!!!! (don)
			#end
